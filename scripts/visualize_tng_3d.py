...
class Interactive3DGalaxyViewer:
    ...
    def _compute_angular_momenta(self):
        ...
        L_s_vec = np.sum(
            weights_s[:, None] * np.cross(coords_s_cen, vel_s_cen), axis=0
        )
        ...
        L_g_vec = np.sum(
            masses_g[:, None] * np.cross(coords_g_cen, vel_g_cen), axis=0
        )
    ...
    def _setup_stellar_plot(self):
        ...
        self.ax_stellar.scatter(
            ...
            c=colors_plot, cmap='viridis', alpha=0.3, s=1, label='Stellar',
    ...
    def _setup_gas_plot(self):
        ...
        self.ax_gas.scatter(
            coords_plot[:, 0], coords_plot[:, 1], coords_plot[:, 2],
            c=colors_plot, cmap='plasma', alpha=0.3, s=1, label='Gas',
            rasterized=True  # Faster rendering
        )
    ...
    def _setup_sfr_plot(self):
        ...
        if len(coords_sf) == 0:
            self.ax_sfr.text(0.5, 0.5, 0.5, 'No star formation', 
                           ha='center', va='center', transform=self.ax_sfr.transAxes)
            return
        ...
        self.ax_sfr.scatter(
            coords_plot[:, 0], coords_plot[:, 1], coords_plot[:, 2],
            c=colors_plot, cmap='hot', alpha=0.4, s=2, label='SFR',
            rasterized=True  # Faster rendering
        )
    ...
    def _add_L_vectors(self):
        ...
        self.ax_stellar.quiver(
            ...
            color='cyan', arrow_length_ratio=0.2, linewidth=3, label='L_stellar'
        )
        ...
        for ax in [self.ax_gas, self.ax_sfr]:
            ax.quiver(
                ...
                color='red', arrow_length_ratio=0.2, linewidth=3, label='L_gas'
            )
        ...
        self.ax_gas.quiver(
            ...
            linestyle='--', alpha=0.5, label='L_stellar'
        )
    ...
    def _print_info(self, event=None):
        ...
        print(f"Gas L vector:     [{self.L_gas[0]:.4f}, "
        ...
        print(f"Catalog gas inclination: "
              f"{self.subhalo['Inclination_gas']:.2f}°")
        print(f"Catalog PA difference: "
    ...
    def create_rotation_animation(self, output_path, n_frames=120, fps=30, 
                                  start_elev=0, start_azim=-90):
        """
        ...
        for widget in [self.slider_elev, self.slider_azim, 
                      self.btn_face, self.btn_edge, self.btn_iso, self.btn_info]:
        ...
        def update(frame):
            ...
            self.ax_stellar.set_title(f'Stellar Particles\nAzimuth: {azim:.1f}°', y=0.85)
...
def main():
    ...
    group.add_argument('--subhalo-id', type=int, help='SubhaloID (8, 17, 19, 20, or 29)')
    
    # Animation options
    parser.add_argument('--animate', action='store_true', 
                       help='Create animation instead of interactive viewer')
    parser.add_argument('--output', type=str, default='tng_rotation.gif',
    ...
    parser.add_argument('--start-elev', type=float, default=0,
                       help='Starting elevation angle (default: 0 = edge-on)')
    parser.add_argument('--start-azim', type=float, default=-90,

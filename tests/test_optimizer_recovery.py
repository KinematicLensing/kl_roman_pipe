...
def generate_synthetic_velocity_data(
    ...
    data_noisy = synth.generate(
        image_pars, snr=snr, seed=config.seed, include_poisson=config.include_poisson_noise
    )
...
def generate_synthetic_intensity_data(
    ...
    data_noisy = synth.generate(
        image_pars, snr=snr, seed=config.seed, 
...
def test_optimize_centered_velocity_base(snr, test_config, velocity_grids):
    ...
    data_true, data_noisy, variance = generate_synthetic_velocity_data(
        CenteredVelocityModel, true_pars, test_config.image_pars_velocity, snr, test_config
    )    # Diagnostic plots
    model_eval = model(theta_true, 'obs', X, Y)
    test_name = f"opt_centered_vel_base_snr{snr}"
    plot_data_comparison_panels(
        data_noisy, data_true, model_eval, test_name, test_config,
        data_type='velocity', variance=variance, n_params=len(model.PARAMETER_NAMES)
    )
    ...
    plot_data_comparison_panels(
        data_noisy, data_true, model_eval_opt, f"{test_name}_optimized", test_config,
        data_type='velocity', variance=variance, n_params=len(model.PARAMETER_NAMES),
    ...
    product_passed, product_stats = check_degenerate_product_recovery(true_pars, pars_opt, snr=snr)
    
    # Create parameter comparison plot
    exclude_params = ['cosi', 'g1', 'g2']
    plot_parameter_comparison(true_pars, pars_opt, recovery_stats, test_name, test_config, snr,
    ...
    if not product_passed:
        ...
        print(f"    True: {product_stats['true']:.2f}, Recovered: {product_stats['recovered']:.2f}")
        print(f"    Rel error: {product_stats['rel_error']:.1%} (tolerance: {product_stats['tolerance']:.1%})")
    
    assert_parameter_recovery(recovery_stats, snr, 'Optimizer: Centered velocity (base)', exclude_params=['cosi', 'g1', 'g2'])
    assert product_passed, f"Degenerate product vcirc*sini not recovered: {product_stats['rel_error']:.1%} error"
@pytest.mark.parametrize("snr", [1000, 50, 10])
def test_optimize_offset_velocity(snr, test_config, velocity_grids):
    ...
    # Generate data
    data_true, data_noisy, variance = generate_synthetic_velocity_data(
        OffsetVelocityModel, true_pars, test_config.image_pars_velocity, snr, test_config
    )    # Diagnostic plots
    model_eval = model(theta_true, 'obs', X, Y)
    ...
    extent = test_config.image_pars_velocity.shape[0] * test_config.image_pars_velocity.pixel_scale / 2
    bounds = [
        (0.1, 0.99),       # cosi
        (0.0, np.pi),      # theta_int
        (-0.1, 0.1),       # g1
    ...
    assert_parameter_recovery(recovery_stats, snr, 'Optimizer: Offset velocity with shear', exclude_params=['cosi', 'g1', 'g2'])
    assert product_passed, f"Degenerate product vcirc*sini not recovered: {product_stats['rel_error']:.1%} error"
# ==============================================================================
...
def test_optimize_inclined_exponential(snr, test_config, intensity_grids):
    ...
    data_true, data_noisy, variance = generate_synthetic_intensity_data(
        InclinedExponentialModel, true_pars, test_config.image_pars_intensity, snr, test_config
    )    # Diagnostic plots
    model_eval = model(theta_true, 'obs', X, Y)
    test_name = f"opt_inclined_exp_snr{snr}"
    plot_data_comparison_panels(
        data_noisy, data_true, model_eval, test_name, test_config,
        data_type='intensity', variance=variance, n_params=len(model.PARAMETER_NAMES)
    )
    ...
    # Bounds
    extent = test_config.image_pars_intensity.shape[0] * test_config.image_pars_intensity.pixel_scale / 2
    bounds = [
        (0.1, 0.99),      # cosi
        (0.0, np.pi),     # theta_int
        (-0.1, 0.1),      # g1
    ...
    exclude_params = ['cosi', 'g1', 'g2']
    plot_parameter_comparison(true_pars, pars_opt, recovery_stats, test_name, test_config, snr,
                             product_stats=None, exclude_params=exclude_params)

    # No vcirc*sini check for intensity-only models (no velocity field)
    assert_parameter_recovery(recovery_stats, snr, 'Optimizer: Inclined exponential intensity', exclude_params=['cosi', 'g1', 'g2'])

...
class TestLaplacePreconditioner:
    ...
    def test_map_matches_independent_scipy(self, joint_model_task):
        """The on-device (optax L-BFGS) MAP must match an independent host-side
        scipy L-BFGS-B MAP on the correlated joint posterior.

        This is the equivalence guard for the JAX-native preconditioner: the
        optimizer backend changed (host scipy loop -> vmapped optax L-BFGS on
        device), the MAP it targets must not. Uses the joint task because that
        is where a weaker on-device optimizer (jax.scipy BFGS) demonstrably
        stalls at the wrong mode.
        """
        from scipy.optimize import minimize

        task, _ = joint_model_task

        # Independent scipy L-BFGS-B MAP, same scaled-coord multi-start recipe.
        vg = task.get_log_posterior_and_grad_fn()
        prior_batch = np.asarray(task.sample_prior(jax.random.PRNGKey(0), 512))
        loc = prior_batch.mean(0)
        prior_scale = np.where(prior_batch.std(0) > 0, prior_batch.std(0), 1.0)

        def neg_u(u):
            v, g = vg(jnp.asarray(loc + prior_scale * u))
            return float(-v), np.asarray(-g, np.float64) * prior_scale

        starts = np.asarray(task.sample_prior(jax.random.PRNGKey(1), 4))
        best = None
        for s0 in starts:
            res = minimize(
                neg_u,
        ...
        scipy_map = loc + prior_scale * best.x

        pre = task.laplace_preconditioner(n_starts=4, seed=0)
        optax_map = np.asarray(pre.map_point)

        # optax must find an equally-good-or-deeper optimum (defining property
        # of the MAP; robust in flat/degenerate directions where the point
        # itself is under-determined).
        lp_scipy = float(task.log_posterior(jnp.asarray(scipy_map)))
        lp_optax = float(task.log_posterior(jnp.asarray(optax_map)))
        assert (
            lp_optax >= lp_scipy - 1e-3
        ), f"optax MAP log-post {lp_optax:.6f} worse than scipy {lp_scipy:.6f}"
        # Well-constrained (SNR=1000) params: agree to <5% of a prior std.
        for i, name in enumerate(task.sampled_names):
            d = abs(optax_map[i] - scipy_map[i]) / prior_scale[i]
            assert d < 0.05, (
                f"{name}: optax MAP {optax_map[i]:.5g} vs scipy {scipy_map[i]:.5g}"
                f" ({d:.3g} prior-std)"
            )

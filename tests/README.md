...
The failure mode this replaces: a tolerance below the noise floor passes
only because of the pinned seed, so a green suite certifies seed luck,
not code. Reading results: a green deterministic gate means the model is
right at the stated accuracy; a green k-sigma check means the estimator
landed within what the data can deliver; a red anything means stop and
root-cause -- never retune the number to make it pass.

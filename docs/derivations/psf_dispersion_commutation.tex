...
(standard PSF-dilution scaling). With flagship-like numbers ---
$\sigma_{\rm psf} \approx 0.076''$ (FWHM $0.18''$), $r_{\rm gal} \approx
0.3''$, so $R^2_{\rm psf}/R^2_{\rm gal} \approx 0.06$ --- and
$\lambda_0 = 1313$\,nm at $z=1$:

\begin{center}
\begin{tabular}{lccc}
\toprule
window & $\delta\lambda$ & $\delta R^2/R^2$ & implied $m$ \\
\midrule
single velocity-broadened line & $\pm1$\,nm & $1.5\times10^{-3}$ & $\sim1\times10^{-4}$ \\
H$\alpha$+[NII] blend span & $\pm2.5$\,nm & $3.8\times10^{-3}$ & $\sim2\times10^{-4}$ \\
full modeled cube window & $\pm13$\,nm & $2.0\times10^{-2}$ & $\sim1.3\times10^{-3}$ \\
\bottomrule
\end{tabular}
\end{center}

Three observations. (i) Per-line constancy is safe by a wide margin: an
implied $m \sim 10^{-4}$ sits far below the paper's ``reasonable
...
cube window (relevant to the continuum, which spans all slices) is marginal
at the $10^{-3}$ level --- this bounds the pre-existing issue-\#51
approximation, not anything introduced by the fast path, but it sharpens the
case for the per-line generalization when requirement-level bias control is
eventually attempted. (iii) For the paper's self-consistent mocks
(fit model $\equiv$ truth model, same PSF assumption on both sides) the bias
is identically zero by construction; the numbers above bound
model-adequacy for real data. An empirical check of the diffraction-limited
scaling against \code{galsim.roman.getPSF} adaptive moments is recorded in

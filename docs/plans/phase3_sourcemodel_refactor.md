...
| pending | 6. move `_apply_post_dispersion_pixel_response` to `kl_pipe/grism.py` (cleanup B) | -- | (renumbered from prev. 5) The function now lives in kl_pipe/model.py:454 and has a new `coarse_pixel_scale` arg + SB→flux conversion. Move is still mechanical: create kl_pipe/grism.py; relocate the function; update 2 callsites (kl_pipe/source.py + kl_pipe/model.py) + 1 test import (tests/test_cube_psf.py). |
| pending | 7. centralize `RenderConfig.for_priors` callsites in `from_obs` (cleanup E) | -- | (renumbered from prev. 6) Likely a no-op or near-no-op after commits 4+5: GrismObs parity landed in 4, grism bandwidth machinery in 5. Original target (2 legacy sites in `from_intensity_model` / `from_joint_model`) is about to be deleted in commit 10. Re-evaluate scope before executing. |

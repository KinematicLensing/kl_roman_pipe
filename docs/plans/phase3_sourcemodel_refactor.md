...
| done | 8. `InferenceTask.from_obs` auto-derives rc + `with_render_config` rebuild (replaces cleanup D) | `00e579d` | The originally-pencilled "Pars/SampledPars/MetaPars dotted-key support" was confirmed unnecessary (PriorDict already absorbs dotted keys). Slot taken by the rc-API ergonomics fix: `build_*_obs` without `render_config` marks the obs `_rc_was_default=True` (private `field(init=False)` on `ImageObs`/`GrismObs`, set via `object.__setattr__`); `InferenceTask.from_obs` detects the flag, derives the priors-sized rc, and rebuilds the obs internally via new public `obs.with_render_config(new_rc, *, int_model=None)` method (recomputes PSF FFT, fine grids, kspace_psf_fft for ImageObs / pixel_response_fft for GrismObs). User-facing inference path drops to one step (no helper call needed). Bespoke 2-step pattern remains supported. Updated `SOURCE_MODEL_EXAMPLES.py`, `docs/tutorials/quickstart.md` with new RenderConfig section + explicit `with_render_config` pedagogical walkthrough. GrismObs per-line PSF / sub-cube limitation tracked as issue #51 with docstring notes on `GrismObs` and `SourceModel.render_grism`. Future "render-time auto-derive" extension documented in `docs/notes/render_config_auto_derive_future.md` (gitignored working-notes location). 5 new tests + 765 passing in make test-basic. |
| done | 8.5. drop cached `obs.image_rotation`; derive from `obs.image_pars.wcs` at render time | -- | Adjacent cleanup: the `image_rotation` field on `ImageObs`/`VelocityObs`/`GrismObs` was a precomputed cache of `image_rotation_from_wcs(image_pars.wcs)`. Removed in favour of calling the helper at render time (once per JIT trace; the value is baked into the traced graph). Touches: 3 dataclass fields dropped, 4 `build_*_obs` constructor kwarg lines dropped, 3 obs pytree aux entries removed, render methods in `kl_pipe/source.py` updated to call `image_rotation_from_wcs(obs.image_pars.wcs)` (or `obs.grism_pars.image_pars.wcs` for grism), `image_rotation_from_wcs` import added in `kl_pipe/source.py` and dropped in `kl_pipe/observation.py`. Test impact: `tests/test_obs_image_rotation.py` repurposed from "cache contract" to "WCS-preservation contract" (assertions now use `image_rotation_from_wcs(obs.image_pars.wcs)`); `tests/test_grism_bandwidth.py` line 351 inlines the helper call. Same 765 passing in make test-basic. |

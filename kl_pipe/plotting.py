...
def create_default_image_pars(
    ...
    ----------
    rmax : float
        Maximum radius from center (in arcsec if pixel_scale provided, else arbitrary units).
    Ngrid : int
        Number of pixels along each axis.
    pixel_scale : float, optional
        Pixel scale in arcsec/pixel. If None, uses 1.0 and interprets rmax in pixels.
    ...
    shape = (Ngrid, Ngrid)
    return ImagePars(shape=shape, pixel_scale=pixel_scale, indexing='ij')


def get_coordinate_grids(image_pars: ImagePars, centered: bool = True) -> Tuple[np.ndarray, np.ndarray]:
    """
    Generate coordinate grids from ImagePars.
    ...
    image_pars : ImagePars
        Image parameters.
    centered : bool
        If True, center the grid at (0, 0). If False, use pixel indices.
        
    Returns
    -------
    X, Y : np.ndarray
        2D coordinate grids in arcsec (if centered) or pixels (if not).
    """
    Ny, Nx = image_pars.shape
    
    if centered:
        # Create grid centered at (0, 0) in arcsec
        x_half = (Nx / 2) * image_pars.pixel_scale
        y_half = (Ny / 2) * image_pars.pixel_scale
...
def plot_velocity_map(
    ...
    **kwargs
    ...
    if image_pars is None:
        ...
        pixel_scale = 2 * rmax / Ngrid
        image_pars = create_default_image_pars(rmax, Ngrid, pixel_scale)
    
    # Get coordinate grids
    X, Y = get_coordinate_grids(image_pars, centered=True)
    
    # Evaluate model
    if speed:
        # For speed, evaluate circular velocity directly
        from kl_pipe.transform import transform_to_disk_plane
        ...
        theta_int = model.get_param('theta_int', theta) if 'theta_int' in model._param_indices else 0.0
        sini = model.get_param('sini', theta) if 'sini' in model._param_indices else 0.0
        
        X_disk, Y_disk = transform_to_disk_plane(plane, x0, y0, g1, g2, theta_int, sini, X, Y)
    ...
    ax.set_xlabel(f'arcsec')
    ax.set_ylabel(f'arcsec')
    ax.set_aspect('equal')
...
def plot_all_planes(
    ...
    """
    from kl_pipe.transform import SUPPORTED_PLANES
...
def plot_rotation_curve(
    ...
    X_arcsec, Y_arcsec = get_coordinate_grids(image_pars, centered=True)
    
    # Also get pixel coordinates for display
    X_pix, Y_pix = get_coordinate_grids(image_pars, centered=False)
    ...
    x0_pix = x0 / image_pars.pixel_scale + image_pars.Nx / 2
    y0_pix = y0 / image_pars.pixel_scale + image_pars.Ny / 2
    ...
    im = ax1.imshow(vmap.T, origin='lower', cmap='RdBu_r', vmin=vmin, vmax=vmax,
                    extent=[0, image_pars.Nx, 0, image_pars.Ny])
    plt.colorbar(im, ax=ax1, label='km/s')
    
    # Mark major axis
    ax1.axline((x0_pix, y0_pix), slope=dy/dx if dx != 0 else np.inf, 
               color='k', ls='-', label='Major axis', lw=1.5)
    ...
    for sign in [-1, 1]:
        ...
        ax1.axline((x_offset, y_offset), slope=dy/dx if dx != 0 else np.inf,
                   color='k', ls=':', alpha=0.5, lw=1)
    
    ax1.set_xlim(0, image_pars.Nx)
    ax1.set_ylim(0, image_pars.Ny)
    ...
    # Rotation curve
    ax2.errorbar(bin_centers, rotation_curve, rotation_curve_err, 
                 marker='o', label='Extracted', capsize=3)
    ax2.axhline(vcirc, c='k', ls='--', label=f'v_circ = {vcirc:.1f}')
    ax2.axhline(-vcirc, c='k', ls='--')
    
    rscale_pix = rscale / image_pars.pixel_scale
    ax2.axvline(rscale_pix, c='g', ls=':', label=f'r_scale')

...
TNG_COSMOLOGY = FlatLambdaCDM(H0=67.74 * u.km / u.s / u.Mpc, Om0=0.3089, Tcmb0=2.725 * u.K)
...
class TNGDataVectorGenerator:
    ...
    def __init__(self, galaxy_data: Dict[str, Dict]):
        ...
        if self.subhalo is None:
            raise ValueError("Subhalo data required for coordinate conversion and orientation")
    ...
    def _get_reference_center(self, center_on_peak: bool, band: str = 'r', use_dusted: bool = True) -> np.ndarray:
        """
    ...
    def _apply_new_orientation(
        ...
        gamma = np.sqrt(g1**2 + g2**2)
        if gamma >= 1.0:
            raise ValueError(f"Shear too large: |g|={gamma:.3f} >= 1.0. Weak lensing requires |g| < 1.")
    ...
    def _grid_particles_cic(
        ...
        Ny, Nx = image_pars.shape if image_pars.indexing == 'ij' else (image_pars.shape[1], image_pars.shape[0])
        ...
        for dx_i, dy_i, w_corner in [(0, 0, w00), (1, 0, w10), (0, 1, w01), (1, 1, w11)]:
            xi = x_floor + dx_i
            yi = y_floor + dy_i
            ...
            if valid.any():
                ...
                np.add.at(weighted_sum, (yi_valid, xi_valid), values_valid * weights_valid * w_valid)
    ...
    def _grid_particles_ngp(
        ...
        H_weighted, _, _ = np.histogram2d(
            y_pix, x_pix, bins=[Ny, Nx], range=[[0, Ny], [0, Nx]],
    ...
    def generate_intensity_map(
        self, config: TNGRenderConfig, snr: Optional[float] = None, seed: Optional[int] = None
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        center = self._get_reference_center(config.center_on_peak, config.band, config.use_dusted)
        ...
        if config.use_native_orientation:
            ...
            if config.pars is None:
                raise ValueError("pars must be provided when use_native_orientation=False")
            ...
            coords_2d, _, _ = self._apply_new_orientation(coords_disk, np.zeros_like(coords_disk), config.pars)
        ...
        if config.use_cic_gridding:
            intensity = self._grid_particles_cic(
                coords_arcsec, luminosities_norm, np.ones_like(luminosities_norm), 
                config.image_pars, mode='sum'
            )
        else:
        ...
        if snr is not None:
            ...
            intensity, variance = add_noise(intensity, target_snr=snr, include_poisson=False, seed=seed)
        else:
    ...
    def generate_velocity_map(
        ...
        if config.use_native_orientation:
            ...
            coords_2d, vel_los, _ = self._apply_new_orientation(coords_disk, velocities_disk, config.pars)
        ...
        if config.use_cic_gridding:
            velocity = self._grid_particles_cic(
                coords_arcsec, vel_los, masses_norm, 
                config.image_pars, mode='weighted_average'
            )
        else:
        ...
        # Add noise if requested
        if snr is not None:
            velocity, variance = add_noise(velocity, target_snr=snr, include_poisson=False, seed=seed)
        else:
    ...
    def generate_sfr_map(
        ...
        if config.use_cic_gridding:
            sfr_map = self._grid_particles_cic(
                coords_arcsec, sfr, np.ones_like(sfr),
                config.image_pars, mode='sum'
            )
        else:
        ...
        # Add noise if requested
        if snr is not None:
            sfr_map, _ = add_noise(sfr_map, target_snr=snr, include_poisson=False, seed=seed)

...
```python
from kl_pipe.tng import TNG50MockData, TNG50Galaxy

# Full-dataset workflow (loads all galaxies into memory)
...
# Low-RAM workflow (loads only one galaxy from split files)
single = TNG50Galaxy(subhalo_id=8)
galaxy = single.get_galaxy()
...
- `TNG50MockData`: convenience class for analyses that need repeated random access to many galaxies and can afford loading full arrays.
- `TNG50Galaxy`: memory-safe class for loading one galaxy at a time from split files (recommended for low-memory notebooks/jobs).

`TNG50Galaxy` expects split data under `.../tng50/gold_split/{gas,stellar,subhalo}/` with `manifest.csv` files.
...
# Load one galaxy from split files (low-RAM friendly)
galaxy = TNG50Galaxy(subhalo_id=8).get_galaxy()
...
Optional split gold-sample layout for low-memory loading can be generated by running `scripts/split_gold_samples.py`
